# Reader context rollout audit

Snapshot: 2026-09-05

This report measures structural orientation signals, not the quality of
the recommendations themselves. A legacy previous/next link counts
separately from the newer three-route continuation pattern.

## Coverage

| Scope | Pages | Articles | Series / collection pages | Breadcrumbs | Three-route continuation | New site shell |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| All physical editions | 7,349 | 6,647 | 702 | 128 | 100 | 0 |
| Canonical-root pages | 3,105 | 2,731 | 374 | 103 | 100 | 0 |
| Independent language editions | 4,244 | 3,916 | 328 | 25 | 0 | 0 |

## Existing article navigation

| Scope | Series position | Legacy previous / next |
| --- | ---: | ---: |
| All physical editions | 3,723 | 5,001 |
| Canonical-root pages | 612 | 1,917 |
| Independent language editions | 3,111 | 3,084 |

## Canonical-root articles by collection

| Collection | Articles | Breadcrumbs | Three-route continuation | Series position | Legacy previous / next |
| --- | ---: | ---: | ---: | ---: | ---: |
| `science-fiction` | 784 | 1 | 1 | 0 | 679 |
| `everyday` | 332 | 1 | 1 | 332 | 332 |
| `literature` | 284 | 0 | 0 | 0 | 190 |
| `film` | 147 | 0 | 0 | 0 | 60 |
| `aesthetics` | 128 | 0 | 0 | 0 | 0 |
| `jiaoyi` | 106 | 0 | 0 | 0 | 105 |
| `mingren` | 104 | 0 | 0 | 0 | 104 |
| `daodejing` | 82 | 0 | 0 | 0 | 0 |
| `tv` | 82 | 0 | 0 | 0 | 82 |
| `anime` | 72 | 0 | 0 | 0 | 0 |
| `wuxia` | 66 | 0 | 0 | 0 | 0 |
| `games` | 51 | 0 | 0 | 0 | 0 |
| `(standalone essays)` | 49 | 4 | 4 | 0 | 21 |
| `hlm` | 48 | 0 | 0 | 0 | 0 |
| `president` | 26 | 0 | 0 | 26 | 26 |
| `emperor` | 25 | 0 | 0 | 25 | 25 |
| `athletics` | 23 | 0 | 0 | 23 | 23 |
| `economy` | 23 | 0 | 0 | 23 | 23 |
| `xiyou` | 23 | 0 | 0 | 0 | 0 |
| `ouya` | 22 | 0 | 0 | 22 | 22 |
| `worldcup` | 22 | 0 | 0 | 22 | 22 |
| `blockchain` | 21 | 21 | 21 | 21 | 21 |
| `sae-foundations` | 16 | 15 | 15 | 15 | 15 |
| `war` | 16 | 0 | 0 | 16 | 16 |
| `sae-nicomachean` | 13 | 13 | 13 | 13 | 13 |
| `sae-republic` | 13 | 13 | 13 | 13 | 13 |
| `method` | 12 | 0 | 0 | 0 | 12 |
| `sae-first-critique` | 12 | 12 | 12 | 12 | 12 |
| `yinan` | 9 | 0 | 0 | 0 | 9 |
| `daode` | 8 | 0 | 0 | 8 | 8 |
| `sae-value` | 8 | 0 | 0 | 8 | 8 |
| `conflict` | 7 | 0 | 0 | 7 | 7 |
| `quanli` | 7 | 0 | 0 | 7 | 7 |
| `rights` | 7 | 0 | 0 | 7 | 7 |
| `econ` | 6 | 0 | 0 | 0 | 6 |
| `sae-buddhist` | 6 | 6 | 6 | 6 | 6 |
| `sae-consolation` | 6 | 6 | 6 | 6 | 6 |
| `anthro` | 5 | 0 | 0 | 0 | 5 |
| `bio` | 5 | 0 | 0 | 0 | 5 |
| `civhist` | 5 | 0 | 0 | 0 | 5 |
| `kimetsu` | 5 | 0 | 0 | 0 | 0 |
| `law` | 5 | 0 | 0 | 0 | 5 |
| `wwii` | 5 | 0 | 0 | 0 | 0 |
| `epistemology` | 4 | 0 | 0 | 0 | 4 |
| `frieren` | 4 | 0 | 0 | 0 | 0 |
| `hulan` | 4 | 0 | 0 | 0 | 0 |
| `learning` | 4 | 0 | 0 | 0 | 4 |
| `psycho` | 4 | 0 | 0 | 0 | 4 |
| `temporal-arts` | 4 | 0 | 0 | 0 | 4 |
| `to-live` | 4 | 0 | 0 | 0 | 0 |
| `never-let-me-go` | 3 | 0 | 0 | 0 | 0 |
| `xiangjun` | 3 | 0 | 0 | 0 | 0 |
| `consciousness-life` | 1 | 1 | 1 | 0 | 1 |

## How to use this report

- Prioritize complete reading paths, not the collections with the largest raw deficit.
- Treat breadcrumbs and continuation cards as editorial infrastructure: links must be reviewed in context.
- Roll independent Japanese, French, German, Spanish, and Korean editions into the shared architecture only after the canonical pattern is stable.
- Do not use coverage percentage as a reason to replace a strong existing series map with generic navigation.
