...
class ZhuangziTests(unittest.TestCase):
    ...
    def test_seventh_batch_editorial_boundaries(self):
        ...
        self.assertIn('essays 30–41 and 59–65 available',self.pages['index.html'])
    ...
    def test_ninth_batch_editorial_boundaries(self):
        ...
        self.assertIn('essays 42–58 available',self.pages['index.html'])
    ...
    def test_eleventh_batch_editorial_boundaries(self):
        for n,zh_marker,en_marker in (
            (51,'他接受了政事，做了三年','He governs for three years'),
            (52,'不是经核实的私人日记','not a verified private diary'),
            (53,'不是死后经验的证据','not evidence about an afterlife'),
            (54,'传本有“三月”和“三日”的差异','versions differ between three days and three months'),
            (55,'不是“那张皮就是罪”','The skin is not the crime'),
        ):
            self.assertIn(zh_marker,(b.DATA/'zh'/f'{n}.md').read_text())
            self.assertIn(en_marker,(b.DATA/'en'/f'{n}.md').read_text())
            self.assertIn(n,b.CHAPTERS)
            tc=(b.DATA/'zh-hant'/f'{n}.md').read_text()
            for error in ('松開','一刻不松','拿不准','賬','消沈','贊美','每只動物','身份','虛己以游世'):
                self.assertNotIn(error,tc)
            ref=485 if n==51 else 482 if n<=53 else 484
            self.assertIn(f'www.chineseclassic.com/content/{ref}',self.pages[f'{n}.html'])
        self.assertIn('入其俗，从其令',(b.DATA/'zh/54.md').read_text())
        self.assertIn('没有交代蝉和螳螂已经被吃掉',(b.DATA/'zh/54.md').read_text())
        self.assertIn('洒心去欲',(b.DATA/'zh-hant/55.md').read_text())
        self.assertIn('虛己以遊世',(b.DATA/'zh-hant/55.md').read_text())
        self.assertNotIn('当然说死了好',self.m['items'][53]['zh_title'])
        self.assertNotIn('就是它的罪',self.m['items'][55]['zh_title'])

    def test_twelfth_batch_editorial_boundaries(self):
        for n,zh_marker,en_marker in (
            (56,'狂屈闻之，以黄帝为知言','The outward shape of an explanation cannot guarantee'),
            (57,'原文没有说明他已领悟','The text does not certify his enlightenment'),
            (58,'探究与表达不是必然相争','Inquiry and expression are not necessarily competitors'),
            (59,'原书没有到这里就停','This is a reading developed here'),
            (60,'原文没有交代撤回刺客或停兵的命令','The text does not record an order withdrawing the assassin'),
        ):
            self.assertIn(zh_marker,(b.DATA/'zh'/f'{n}.md').read_text())
            self.assertIn(en_marker,(b.DATA/'en'/f'{n}.md').read_text())
            self.assertIn(n,b.CHAPTERS)
            tc=(b.DATA/'zh-hant'/f'{n}.md').read_text()
            for error in ('知北游','覈實','松開','這隻是','划定','答復','贊美','贊嘆','身份','付賬','沈重','計劃'):
                self.assertNotIn(error,tc)
            self.assertIn(f'www.chineseclassic.com/content/{486 if n<=58 else 489}',self.pages[f'{n}.html'])
        self.assertIn('沉默本身不替这些原因作出选择',(b.DATA/'zh/56.md').read_text())
        self.assertIn('没有把道换到最下面',(b.DATA/'zh/57.md').read_text())
        self.assertIn('Invisibility is not a duty',(b.DATA/'en/58.md').read_text())
        self.assertIn('郭象把它解释为每日与物俱化，所以常无我',(b.DATA/'zh/59.md').read_text())
        self.assertIn('闔嘗舍之',(b.DATA/'zh-hant/59.md').read_text())
        self.assertIn('https://zh.wikisource.org/wiki/莊子/齊物論',self.pages['59.html'])
        self.assertIn('carefully directed analogy',(b.DATA/'en/60.md').read_text())
        self.assertNotIn('只有最后一个劝动了',self.m['items'][60]['zh_title'])
        self.assertIn('42—58 已可读 · 本辑完整',self.pages['index.html'])

    def test_thirteenth_batch_editorial_boundaries(self):
        for n,zh_marker,en_marker in (
            (61,'一个“销”字不能证明他曾经显赫','the word does not prove a previous career of celebrity'),
            (62,'没有交代每次由什么触发','Being persuaded by evidence is not a lesser achievement'),
            (63,'完整的句子不只“亦明矣”','He explicitly states a conclusion'),
            (64,'不能证明本篇出自或不出自庄周','Style alone cannot prove'),
            (65,'不等最后一篇再结账','rather than postpone it entirely until the final essay'),
        ):
            self.assertIn(zh_marker,(b.DATA/'zh'/f'{n}.md').read_text())
            self.assertIn(en_marker,(b.DATA/'en'/f'{n}.md').read_text())
            self.assertIn(n,b.CHAPTERS)
            ref={61:489,62:489,63:490,64:472,65:491}[n]
            self.assertIn(f'www.chineseclassic.com/content/{ref}',self.pages[f'{n}.html'])
            tc=(b.DATA/'zh-hant'/f'{n}.md').read_text()
            for error in ('旅捨','鄰捨','沈下去','答復','身份','覈實','贊美','賬','計劃','巨鈎','臧與谷','啓發'):
                self.assertNotIn(error,tc)
        self.assertIn('不如舍之以從其志',(b.DATA/'zh-hant/61.md').read_text())
        self.assertIn('留白有时让人慢慢想，有时也会使人困惑',(b.DATA/'zh/61.md').read_text())
        self.assertIn('https://zh.wikisource.org/wiki/莊子/寓言',self.pages['62.html'])
        self.assertIn('小大各有所适',(b.DATA/'zh/63.md').read_text())
        self.assertIn('those earlier errors',(b.DATA/'en/62.md').read_text())
        self.assertIn('以一正万',(b.DATA/'zh/64.md').read_text())
        self.assertIn('大白若辱，盛德若不足',(b.DATA/'zh/65.md').read_text())
        self.assertIn('Neither supplies a verified count',(b.DATA/'en/65.md').read_text())
        self.assertNotIn('四个字',self.m['items'][63]['zh_title'])
        self.assertIn('30—41、59—65 已可读 · 本辑完整',self.pages['index.html'])
