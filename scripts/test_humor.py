...
class HumorTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.data = json.loads(build.DATA.read_text())
        cls.outputs = build.render_pages()

    def test_curated_inventory(self):
        issues = self.data['issues']
        self.assertEqual(len(issues), 5)
        self.assertEqual([len(x['jokes']) for x in issues], [6] * 5)
        jokes = [j for issue in issues for j in issue['jokes']]
        self.assertEqual(len({j['id'] for j in jokes}), 30)
        self.assertEqual(len({j['origin'] for j in jokes}), 30)
        for joke in jokes:
