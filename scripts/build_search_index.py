...
def extract_recent_fiction_descriptions(source: str) -> dict[str, str]:
    ...
    for match in re.finditer(r"<p\b([^>]*)>(.*?)</p>", source, flags=re.I | re.S):
        ...
        if not classes.intersection({"rf-article-deck", "rf-hero-deck", "nf-article-deck", "nf-hero-deck", "books-hero-deck", "drama-search-deck", "western-search-deck", "method-search-deck"}):
            continue
