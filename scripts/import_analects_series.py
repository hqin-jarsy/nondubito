#!/usr/bin/env python3
"""Build The Analects, Reopened in English, Simplified and Traditional Chinese."""
...
def index_html(converter: TraditionalConverter) -> str:
    ...
    for lang, title, deck, note in [
        ...
        author = "Han Qin (秦漢)" if lang == "hant" else "Han Qin (秦汉)"
        sections.append(f'''<div class="lang-{lang} local"><section class="series-hero"><p class="series-kicker">Non Dubito · Chinese Classics</p><h1>{title}</h1><p class="series-deck">{deck}</p><div class="series-meta">{author} · 2026 · 4 movements · 50 essays · EN / 简 / 繁</div></section><aside class="method-note"><strong>{"A note on the reading" if lang == "en" else ("关于这套读法" if lang == "zh" else "關於這套讀法")}</strong><p>{note}</p></aside><section class="reading-route"><h2>{route_title}</h2><p>{route_desc}</p><div class="route-grid">{"".join(routes[lang])}</div></section>{cards(lang, converter)}<section class="research"><header><p>SAE · Analects</p><h2>{research_title}</h2><span>{research_desc}</span></header><div class="research-grid">{research_cards}</div></section></div>''')
    ...
    return f'''<!DOCTYPE html><html lang="en" data-lang="en">{head("The Analects, Reopened · Fifty Encounters with Confucius", "Fifty essays put the names and scenes back before ‘The Master said,’ reopening the Analects as encounters rather than a book of slogans.", "essays/analects/", "website")}<body class="site-shell-page explicit-hant">{shell_header("../../")}<main class="analects-wrap">{"".join(sections)}</main><footer class="site-shell-footer"><div class="site-shell-footer-inner"><div><div class="site-shell-footer-mark">Non <span>Dubito</span></div><p>A mind in many languages</p></div><div class="site-shell-footer-links"><a href="../../about.html">About</a><a href="../../library.html">Full Library</a><a href="https://self-as-an-end.net">SAE Theory ↗</a></div></div></footer><script type="application/ld+json">{schema}</script></body></html>'''
